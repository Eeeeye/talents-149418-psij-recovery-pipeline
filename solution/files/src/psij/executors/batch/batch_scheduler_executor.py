...
class BatchSchedulerExecutor(JobExecutor):
    ...
    *script output file*.
    ...
    def _create_script_context(self, job: Job) -> Dict[str, object]:
        ...
        assert job.spec is not None
        if job.spec.attributes is not None and job.spec.attributes.duration is not None:
            ctx['formatted_job_duration'] = self._format_duration(job.spec.attributes.duration)
        return ctx

    def _format_duration(self, duration: timedelta) -> str:
        """Format a duration as total-hours:minutes:seconds."""
        total = int(math.ceil(duration.total_seconds()))
