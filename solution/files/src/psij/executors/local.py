...
class _ProcessReaper(threading.Thread):
    ...
    def _reset_after_fork(cls) -> None:
        # Locks and Thread objects copied by fork belong to the parent process.
        cls._instance = None
    ...
    def get_instance(cls: Type['_ProcessReaper']) -> '_ProcessReaper':
        ...
        with cls._lock:
            if (cls._instance is None or cls._instance_pid != process_id
                    or not cls._instance.is_alive()):
                cls._instance = _ProcessReaper()
                cls._instance_pid = process_id
                cls._instance.start()
...
if hasattr(os, 'register_at_fork'):
    os.register_at_fork(after_in_child=_ProcessReaper._reset_after_fork)
